...
unsigned char UPLOAD_BUFF[ULOAD_BUFF_SIZE];//转义完成后的数组，上传的网关的数组
unsigned char PROTOCOL_SEND_BUFF[ULOAD_BUFF_SIZE];//将数据摆在这个数组中，然后做转义

//unsigned char graph_buff[3000];//将数据摆在这个数组中，然后做转义

unsigned char PROTOCOL_RECV_BUFF[200];
...
{.function_code = CTRL_SAVE_SYSCFG,			.cmd_length = 0,.function = CMD_CtrlSystemSaveCFG,.help = "protocol explain Demo"},//协议解析例子
...
{
{.function_code = PROTOCOLDEMO,			.cmd_length = 0,.function = CMD_Demo,.help = "protocol explain Demo"},//协议解析例子
{.function_code = FUNCTION_TEST,			.cmd_length = 0,.function = McuFunctionTest,.help = "protocol explain Demo"},//测试函数

{.function_code = CTRL_SENSOR_NET,			.cmd_length = 0,.function = CMD_CtrlSensorNet,.help = "protocol explain Demo"},//控制节点板
{.function_code = GET_SENSOR_UID,			.cmd_length = 0,.function = DealUploadDeviceInfo,.help = "protocol explain Demo"},//获取节点板UID
{.function_code = GET_SENSOR_INFO,			.cmd_length = 0,.function = DealUploadSensorInfo,.help = "protocol explain Demo"},//获取传感器信息
{.function_code = DEVICE_ACK,			.cmd_length = 0,.function = DealDeviceAck,.help = "protocol explain Demo"},//处理节点板的回复

};

S_Cmd RS485_CMD_List[] = 
{
{.function_code = PROTOCOLDEMO,			.cmd_length = 0,.function = CMD_Demo,.help = "protocol explain Demo"},//协议解析例子

{.function_code = RS485CMD_GET_SENSOR_INFO,			.cmd_length = 0,.function = CMD_Demo,.help = "protocol explain Demo"},//发送开始扫描命令
};
	...
	switch(s_cmd)
		{
			case CTRL_SENSOR_NET_UPLOAD_MOD:
				...
				SystemCFG.sensor_upload_mod = sensor_upload_mod;
			...
			case CTRL_SENSOR_NET_SET_CFG:
				...
				SystemCFG.G_CsdParIdacMin = G_CsdParIdacMin;
				SystemCFG.G_CsdParIdacMax = G_CsdParIdacMax;
				SystemCFG.G_CsdParIdac_CMin = G_CsdParIdac_CMin;
				SystemCFG.G_CsdParIdac_CMax = G_CsdParIdac_CMax;
				SystemCFG.G_CsdParSclkMin = G_CsdParSclkMin;
				SystemCFG.G_CsdParSclkMax = G_CsdParSclkMax;
				SystemCFG.G_CsdParWindow_Min = G_CsdParWindow_Min;
				SystemCFG.G_CsdParWindow_Max = G_CsdParWindow_Max;
			...
			case CTRL_SENSOR_NET_SET_ALL_BASELINE:
				...
				SystemCFG.G_BASELINE_THRESHOLE = G_BASELINE_THRESHOLE;
				SystemCFG.G_BASELINE_NOISE_THRESHOLE = G_BASELINE_NOISE_THRESHOLE;
				SystemCFG.G_BASELINE_UPDATA_TIME = G_BASELINE_UPDATA_TIME;
...
{
	HAL_GPIO_WritePin(POWER_CTRL_GPIO_Port,POWER_CTRL_Pin,GPIO_PIN_RESET);//断电
	osDelay(200);
	NVIC_SystemReset();
	return 1;
}
...
void EnterDebugMod(void)
{
//7E 08 ff 00 03 09 EA 00 00 7F
unsigned char buf[10] = {0X7E,0X08,0Xff,0X00,0X03 ,0X09 ,0XEA ,0X00 ,0X00 ,0X7F};
SendToCOM3(buf, 10);
}

void GetDeviceInfo(void)
{
//7e 0a ff 00 01 02 00 ff ff 7f
unsigned char buf[10] = {0X7E,0X0A,0Xff,0X00,0X01 ,0X02 ,0X00 ,0XFF ,0XFF ,0X7F};
SendToCOM3(buf, 10);
...
unsigned char Deal_RS485_Data(unsigned char* data, unsigned short data_length)
{
	...
	//operat_res = DecodeCtrlFrame(*operat_data,data_operat_length, &Rs232frame);	
	operat_res = DecodeCtrlFrame(data,(data_operat_length), &Rs485frame);
	...
	#endif
		{
			return(ExecuteFrame(&Rs485frame, RS485_CMD_List, (sizeof(RS485_CMD_List)/(sizeof(RS485_CMD_List[0])))));
		}

	if(2 == operat_res)//粘包
		{
			ExecuteFrame(&Rs485frame, RS485_CMD_List, (sizeof(RS485_CMD_List)/(sizeof(RS485_CMD_List[0]))));			
			(*operat_data) += (Rs485frame.Frame_Length.short_ + 1);
			data_operat_length = (Rs485frame.Untreated_length);
			goto Start_Decode_Data;
		}
...
unsigned char Deal_CAN_Data_Fifo(void* can_fifo,unsigned char write_point)
{
	...
	//CheckCanFrame(tmp_can_fifo,write_point);
	...
	//(tmp_can_fifo->RECV_DATA[write_point].ID) = 0;
	...
	//(tmp_can_fifo->RECV_DATA[write_point].ID) = 0XFFFFFFFF;
	
	/*for(loopx = 0 ; loopx < MAX_CAN_FIFO_LENGTH ; loopx ++)
		{
			if(1 == (tmp_can_fifo->RECV_DATA[loopx].Recv_OK))
				{
					(tmp_can_fifo->RECV_DATA[loopx].Recv_OK) = 0;
					Deal_CAN_Data((tmp_can_fifo->RECV_DATA[loopx].RecvBuff),(tmp_can_fifo->RECV_DATA[loopx].Data_length),(tmp_can_fifo->RECV_DATA[loopx].ID));
					(tmp_can_fifo->RECV_DATA[loopx].Recv_ING) = 1;
				}
		}*/
	return 1;
}
...
unsigned char Board_Arrange_1[BOARD_SENSOR_NUM] = {1,0};
	...
	for(loop_all_board = 0 ; loop_all_board < BOARD_Num ; loop_all_board ++)
	{
		...
		*(buff + 19 + loop_all_board*(2+BOARD_SENSOR_NUM*2) ) = BOARD_SENSOR_NUM*2;		
		...
		for(loop_all_sensor = 0 ;loop_all_sensor < BOARD_SENSOR_NUM ; loop_all_sensor ++)
			{				
				Short_value.short_ = tmp_board->SensorBoard[loop_all_board].sensor[Board_Arrange_1[loop_all_sensor]].Data;
...
{
	static P_SensorNet_Info tmp_net = NULL;
	 		...
	 		if(0XF0 == tmp_net->SensorBoard[loopx].Addr)//新节点板
	 			{
	 				...
	 				DEBUG_SYS_ErrLog("________________%X NEW Node Recv space->%d",(tmp_net->SensorBoard[loopx].BoardUID),loopx);
...
void StartNextScan(unsigned char Menber_num)
{
	...
	//can_send_data[1] = SendLength.char_.char_l;
	can_send_data[0] = CMD;
	can_send_data[1] = CTRL_SENSOR_BOARD_CHOOSE_NEXT_SCAN_GROUP_MEMBER;
	can_send_data[2] = Menber_num;
...
{
	static unsigned char Cur_Board_Group_Addr_Num = 0;
	StartNextScan(Cur_Board_Group_Addr_Num);
	//StartNextScan(1);
	Cur_Board_Group_Addr_Num ++;
	Cur_Board_Group_Addr_Num = Cur_Board_Group_Addr_Num % BOARD_CROUP_NUM;	
}
...
void GetAllSensorValue(unsigned char data_type)
{
	...
	//static unsigned char Cur_Board_Group_Addr_Num = 0;
	//static unsigned char Cur_Board_Group_Delay = 0;
	
	for(loop_all_sensor = 0 ; loop_all_sensor < SensorNet.CurSensorNum ; loop_all_sensor++)
		{
		/*Cur_Board_Group_Delay += CUR_SENSOR_DELAY;
		if(Cur_Board_Group_Delay > BOARD_SCAN_TIME)//当前节点板的组成员扫描完毕 开始组内下一个成员的扫描
			{
				Cur_Board_Group_Delay = 0;
				StartNextScan(Cur_Board_Group_Addr_Num);
				Cur_Board_Group_Addr_Num ++;
				Cur_Board_Group_Addr_Num = Cur_Board_Group_Addr_Num % BOARD_CROUP_NUM;				
...
void upload_diffvalue_232_all_nod_32sensor(void)
{
	...
	U_Short_char sensor_value;
	static int frame = 0;
	//for(loop4 = 0 ; loop4 < (SensorNet.CurSensorNum + (BOARD_SENSOR_NUM*4)) ; loop4 ++)
	for(loop4 = 0 ; loop4 < (SensorNet.CurSensorNum ) ; loop4 ++)
		{
			for(loopx = 0 ; loopx < BOARD_SENSOR_NUM; loopx ++)
				{
					sensor_value.short_ = SensorNet.SensorBoard[loop4].sensor[loopx].Data;
	...
	SendDataByNewProtocol(0x55,frame++,UPLOAD_BUFF,((BOARD_SENSOR_NUM*2)*(SensorNet.CurSensorNum)+11));
}
